<template>
  <div>
    ...
    <div v-if="gameStore.isLoading" class="flex justify-center items-center h-96">
      <div class="text-center">
        ...
        <p class="mt-3 text-lg font-medium text-gray-700 dark:text-gray-300">Loading game...</p>
      </div>
    </div>
    ...
    <div v-else>
        ...
        <div class="flex flex-wrap items-center justify-between">
          ...
          <div class="flex space-x-3">
            <div v-for="player in multiplayerStore.players" :key="player.userId" 
              ...
              <div class="ml-2">
                <span class="font-medium text-sm text-gray-800 dark:text-white">{{ player.username }}</span>
                <div class="text-xs">
                  <span v-if="hasPlayerSubmittedGuess(player.userId)" class="text-green-500">Guessed</span>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div class="relative rounded-lg overflow-hidden h-[400px] lg:h-[600px] bg-gray-100 dark:bg-gray-700">
          <!-- Replace with your actual map component -->
          <GoogleStreetView v-if="gameStore.isRoundActive" />
          <div v-else-if="gameStore.isGameOver" class="p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
            <h2 class="text-xl font-bold text-center mb-4">Game Over!</h2>
            <div class="max-w-3xl mx-auto">
              <MultiplayerResults />
            </div>
          </div>
          <div v-else-if="multiplayerStore.waitingForOtherPlayers" class="p-4 text-center">
            <p class="text-lg font-medium mb-2">Waiting for other players to submit their guesses...</p>
        ...
        <div v-if="gameStore.isRoundActive" class="rounded-lg overflow-hidden h-[400px] lg:h-[600px] bg-gray-100 dark:bg-gray-700">
          <GuessMap @guess-selected="handleGuess" />
      ...
      <div v-if="!gameStore.isGameOver" class="mt-4 bg-gray-50 dark:bg-gray-800 p-3 rounded-lg">
        <h3 class="text-lg font-semibold mb-2 text-gray-800 dark:text-white">Game Chat</h3>
        <div class="h-32 overflow-y-auto mb-2 bg-white dark:bg-gray-700 rounded-md p-2">
          <div v-for="(message, index) in multiplayerStore.chatMessages" :key="index" class="mb-1">
            <span class="font-semibold text-sm text-gray-700 dark:text-gray-300">{{ message.username }}:</span>
            <span class="text-sm text-gray-800 dark:text-white ml-1">{{ message.content }}</span>
          </div>
          <div v-if="multiplayerStore.chatMessages.length === 0" class="text-gray-400 text-center text-sm italic">
            No messages yet
          </div>
        </div>
        <div class="flex">
          <input v-model="chatMessage" type="text" placeholder="Send a message..." 
      ...
      <div class="mt-4 text-center">
        <button @click="leaveGame" class="px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-md">
          Leave Game
        </button>
      </div>
    </div>
