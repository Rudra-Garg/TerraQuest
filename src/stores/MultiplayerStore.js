...
export const useMultiplayerStore = defineStore("multiplayer", () => {
   ...
   *  - Rejects immediately (and logs the user out) if the stored token is already expired.
   *  - Automatically attaches the Authorization header.
   *  - Calls logout + redirects to /login when the server returns 401.
   */
  async function authedFetch(url, options = {}) {
    if (authStore.isTokenExpired) {
      console.warn("MultiplayerStore: stored token is expired. Logging out.");
      authStore.logout();
      router.push("/login");
    ...
    if (response.status === 401) {
      console.warn(
        "MultiplayerStore: received 401 – token rejected by server. Logging out.",
      );
      authStore.logout();
  ...
  const haveAllPlayersGuessedCurrentRound = computed(() => {
    ...
    const activePlayersInLobby = players.value.filter(
      (p) => p.status === "active",
    );
    if (activePlayersInLobby.length === 0) return true; // No one to wait for

    return activePlayersInLobby.every(
      (player) =>
        playerRoundResults.value[player.userId]?.[currentRoundNum - 1]
          ?.guess !== undefined,
    );
  });
  ...
  /**
   * Fetches the current multiplayer game state from the REST endpoint.
   * Used for reconnection/state recovery when WebSocket drops.
   */
  async function fetchGameState() {
    if (!gameId.value) {
      console.warn("fetchGameState: No gameId available.");
      return null;
    }
    ...
    try {
      const response = await authedFetch(
        `${baseUrl}/api/v1/multiplayer/game/${gameId.value}`,
        {
          method: "GET",
        },
      );
      ...
      if (!response.ok)
        throw new Error(
          data.error || `Failed to fetch game state (HTTP ${response.status})`,
        );

      // Update store with server state
      gameCode.value = data.gameCode || gameCode.value;
      roundsTotal.value = data.roundsTotal || roundsTotal.value;
      initialHealth.value = data.initialHealth || initialHealth.value;
      roundDurationSeconds.value =
        data.roundDurationSeconds || roundDurationSeconds.value;

      console.log("fetchGameState: Recovered game state from server:", data);
      return data;
    } catch (err) {
  ...
  function getWebSocketUrl(code) {
    if (!code) return null;
    if (authStore.isTokenExpired) {
      console.warn("Multiplayer Connect: token is expired. Logging out.");
      authStore.logout();
      router.push("/login");
      return null;
    }
    ...
    try {
      ...
      socket.value.onclose = (event) => {
        ...
        if (event.code !== 1000 && event.code !== 1005) {
          // 1005: No status Rcvd (browser close)
          error.value = `Connection closed unexpectedly (Code: ${event.code}). Attempting to reconnect...`;
          // Attempt reconnection after a short delay
          setTimeout(async () => {
            if (gameCode.value && !isConnected.value) {
              ...
              if (
                ...
                error.value = "Game has ended or could not recover state.";
              }
            }
          }, 2000);
        } else {
    ...
    switch (message.type) {
      case "player_list_update":
        if (Array.isArray(message.payload)) {
          ...
          const currentUser = players.value.find(
            (p) => p.userId === authStore.userProfile?.id,
          );
      ...
      case "game_start":
        if (message.payload && message.payload.gameDbId) {
          ...
          roundDurationSeconds.value =
            message.payload.roundDurationSeconds || 60;
      ...
      case "round_start":
        if (message.payload && message.payload.currentRound) {
          console.log(
            `Store: Received round_start for round ${message.payload.currentRound}`,
          );
          showRoundResultsSummary.value = false;
          ...
          roundDurationSeconds.value =
            message.payload.roundDurationSeconds || roundDurationSeconds.value; // Update if sent
          ...
          if (message.payload.location) {
            ...
            if (!gameStore.locations[message.payload.currentRound - 1]) {
              gameStore.locations[message.payload.currentRound - 1] =
                newLocation;
            }
            console.log(
              `Store: Added location for round ${message.payload.currentRound}:`,
              newLocation,
            );
          }
          ...
          console.log(
            `Store: GameStore now at round ${gameStore.currentRoundNumber}, active: ${gameStore.isRoundActive}`,
          );
        }
        break;
      ...
      case "first_player_guessed": // <<< NEW
        if (
          message.payload &&
          message.payload.roundNumber === gameStore.currentRoundNumber
        ) {
      ...
      case "guess_submit": // Individual guess confirmation
        if (message.payload) {
          const { userId, username, roundNumber, score, totalScore } =
            message.payload;
          console.log(
            `Store: Received guess_submit confirmation for ${username} (User ${userId}), Round ${roundNumber}, Score: ${score}, New Total: ${totalScore}`,
          );
          ...
          if (!playerRoundResults.value[userId]) {
            playerRoundResults.value[userId] = new Array(
              roundsTotal.value,
            ).fill(null);
          }
      ...
      case "round_end":
        if (message.payload && message.payload.playerRoundResults) {
          ...
          currentRoundMultiplier.value =
            message.payload.currentMultiplier || 1.0;
          ...
          message.payload.playerRoundResults.forEach((pResult) => {
            if (!playerRoundResults.value[pResult.userId]) {
              ...
              playerRoundResults.value[pResult.userId] = new Array(
                roundsTotal.value,
              ).fill(null);
            }
            playerRoundResults.value[pResult.userId][
              message.payload.roundNumber - 1
            ] = {
              guess: pResult.guess,
            ...
            const playerInList = players.value.find(
              (p) => p.userId === pResult.userId,
            );
            if (playerInList) {
      ...
      case "game_end":
        if (message.payload && message.payload.finalStandings) {
          ...
          players.value = message.payload.finalStandings.map((p) => ({
            ...
            username: p.username,
            isHost:
              players.value.find((pl) => pl.userId === p.userId)?.isHost ||
              false,
            isReady: false,
      ...
      default:
        console.warn(
          "Store: Received unhandled WS message type:",
          message.type,
        );
    }
  }
  ...
  function hostProceedToNextRound() {
    if (!isHost.value) {
      console.error(
        "MultiplayerStore: Non-host tried to proceed to next round.",
      );
      return;
    }
  ...
  function submitGuess() {
    ...
    if (!gameStore.currentGuess) {
      error.value = "Make a guess first.";
      return;
    }
    if (!gameStore.isRoundActive) {
      console.warn("Guess submitted when round not active.");
      return;
    }

    console.log(
      "MultiplayerStore: Submitting guess via WS:",
  ...
  async function createGame(options) {
    ...
    try {
      const response = await authedFetch(
        `${baseUrl}/api/v1/multiplayer/create`,
        {
          method: "POST",
          headers: {
      ...
      if (!response.ok)
        throw new Error(
          data.error || `Failed to create game (HTTP ${response.status})`,
        );
  ...
  async function joinGame(code) {
    ...
    try {
      const response = await authedFetch(
        `${baseUrl}/api/v1/multiplayer/join/${code}`,
        {
          method: "POST",
        },
      );
      ...
      if (!response.ok)
        throw new Error(
          data.error || `Failed to join game (HTTP ${response.status})`,
        );
  ...
  function leaveGame() {
    ...
    gameStore.resetState(); // Also reset single-player game store state
  }
  ...
  return {
    ...
    roundsTotal,
    initialHealth,
    roundDurationSeconds, // <<< EXPOSE
    playerRoundResults,
    currentRoundActualLocation,
    currentRoundWinnerId,
    currentRoundMultiplier,
    showRoundResultsSummary,
    firstPlayerGuessedInRound, // <<< EXPOSE
    isWaitingForRoundEnd, // <<< EXPOSE

    // Computed
    isCurrentPlayerReady,
    isEveryoneReady,
    haveAllPlayersGuessedCurrentRound,
