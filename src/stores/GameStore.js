...
export const useGameStore = defineStore("game", () => {
  ...
  const getCurrentLocation = computed(() => {
    if (
      ...
      currentRound.value <= locations.value.length
    ) {
  ...
  const getCurrentRoundResult = computed(() => {
    if (
      currentRound.value > 0 &&
      currentRound.value <= roundResults.value.length
    ) {
  ...
  const hasSubmittedGuessForCurrentRound = computed(() => {
    ...
    return (
      roundResults.value.length >= currentRound.value && currentRound.value > 0
    );
  });
  ...
  async function startGame() {
    ...
    try {
      ...
      console.log(
        `Store: Game ${gameId.value} started, ${locations.value.length} locations loaded from backend.`,
      );
  ...
  async function nextRound() {
    ...
    if (currentRound.value >= locations.value.length) {
      // Check against actual loaded locations count
  ...
  async function submitGuess() {
    if (
      !currentGuess.value ||
      !isRoundActive.value ||
      hasSubmittedGuessForCurrentRound.value
    ) {
    ...
    if (!actualLocation) {
      console.error(
        "Cannot submit guess, actual location not found for round.",
      );
      return;
    }

    console.log(
      "Store: Submitting guess",
      currentGuess.value,
      "for actual",
      actualLocation,
    );
    ...
    const distance = calculateDistance(
      actualLocation.lat,
      actualLocation.lng,
      currentGuess.value.lat,
      currentGuess.value.lng,
    );
    ...
    // Check if game is over after this round, and if so, submit results
    if (currentRound.value >= locations.value.length) {
      await finishGame();
    }
  ...
  async function finishGame() {
    ...
    const payload = {
      ...
      rounds: roundResults.value.map((r) => ({
        roundNumber: r.round,
        locationId: locations.value[r.round - 1]?.id || 0,
    ...
    try {
      const result = await gameService.finishGame(payload);
      console.log(
        "Store: Game results saved to backend. Server gameId:",
        result.gameId,
      );
    } catch (error) {
      console.error(
        "Store: Failed to save game results to backend:",
        error.message,
      );
      // Don't block the UI — game over screen still shows
    }
  }
